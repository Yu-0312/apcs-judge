...
> 從零基礎到進階：Python · C++ · C · Java 四語言、95 章互動教學，搭配 300 題 APCS 分級題庫與 687 題程式判讀練習——全部在瀏覽器內完成，零安裝、零後端、零註冊。
> Learn to code from zero in four languages — 95 interactive chapters, a 300-problem tiered APCS judge, and a 687-question code-reading drill, entirely in the browser. No install, no backend, no signup.
...
| 🔍 **reading.html** | **判讀：程式識讀** | 687 題「讀程式碼、選答案」的判讀練習，依 APCS 四級難度分庫、Python/C++/JavaScript/C 各語言獨立題庫，並可隨機抽題（最多 50 題），作答後即時對解＋考點解析 |
...
- **687 題**：APCS 官方範例題與統測歷屆判讀題（標「官方解答」）、依考點自編的精選練習題，再加上 **Python / C++ / JavaScript / C 各語言獨立題庫**（每語言約 100 題）；涵蓋程式追蹤、除錯、複雜度、位元、指標、結構與類別、資料結構與演算法、各語言語法陷阱與標準庫等
...
| 🔍 **reading.html** | **Code reading** | 687 "read the code, pick the answer" comprehension questions across four APCS difficulty tiers with separate Python/C++/JavaScript/C banks, plus per-level random draw (up to 50), instant reveal and per-question explanations |
