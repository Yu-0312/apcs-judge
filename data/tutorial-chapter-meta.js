...
const CHAPTER_META = {
 ...
 "98": {
  "learn": [
   "讀錯誤訊息的檔案、行號與錯誤類型",
   "分辨 SyntaxError / NameError / TypeError / IndexError / ValueError",
   "知道 C/C++/Java 常見編譯與執行錯誤的修法"
  ],
  "try": [
   "把範例裡的 <code>score</code> 故意拼成 <code>scroe</code>，觀察 NameError",
   "刪掉一個右括號，觀察 SyntaxError 指到哪一行",
   "修第一個錯誤後再執行，看看後面的錯誤是否一起消失"
  ],
  "quiz": [
   {
    "q": "讀錯誤訊息時，通常應該先處理哪一個？",
    ...
    "ans": 1,
    "why": "第一個錯誤常會連鎖造成後面很多假錯誤，先修它最有效。"
   },
   {
    "q": "Python 的 NameError 最常代表什麼？",
    "opts": [
     "變數或函式名稱不存在",
     "陣列越界",
     "除以 0",
    ...
    "ans": 0,
    "why": "NameError 通常是拼字錯、作用域錯，或使用前尚未指定。"
   }
  ],
  "code": {
   "task": "修正右邊程式的變數名稱錯誤，讓它輸出 <code>100</code>。",
   "stdin": "",
   "expect": "100"
  }
 },
 "99": {
  ...
  "try": [
   "在迴圈前後各加一行 print，觀察 total 的變化",
   "把 nums 改成空串列，看看程式是否仍正常",
   "故意把 <code>total += x</code> 改成 <code>total = x</code>，用輸出找出錯在哪"
  ],
  "quiz": [
   {
    "q": "最小可重現範例的目的？",
    ...
    "ans": 1,
    "why": "除錯時先縮小範圍，才能更快定位真正原因。"
   },
   {
    "q": "二分定位 bug 的核心想法是？",
    "opts": [
     "一次改很多地方",
     "在中間設 checkpoint，把問題分成前半與後半",
     "只看最後一行",
     "把所有變數都改名"
    ],
    "ans": 1,
    "why": "像二分搜尋一樣，每次把可能出錯的範圍砍半。"
   }
  ],
  "code": {
   "task": "右邊程式應該輸出串列總和。修正 bug，讓輸出為 <code>14</code>。",
   "stdin": "",
   "expect": "14"
  }
 },
 "100": {
  ...
  "try": [
   "stdin 輸入 <code>abc</code>，確認程式不會崩潰",
   "stdin 輸入 <code>150</code>，確認會顯示超出範圍",
   "stdin 輸入 <code>88</code>，確認會顯示合法分數"
  ],
  "quiz": [
   {
    "q": "為什麼不一定要直接寫 <code>score = int(input())</code>？",
    "opts": [
     "int() 永遠不能用",
     "遇到非數字輸入會 ValueError",
     "會讓數字變小",
    ...
    "ans": 1,
    "why": "真實使用者可能輸入 abc、空字串或其他格式，先檢查可避免崩潰。"
   },
   {
    "q": "函式 <code>average(nums)</code> 先檢查空串列，是為了避免？",
    "opts": [
     "除以 0",
     "語法錯誤",
     "變數拼錯",
    ...
    "ans": 0,
    "why": "空串列長度為 0，直接 sum(nums) / len(nums) 會除以 0。"
   }
  ],
  "code": {
   "task": "讀入一行，如果是 0–100 的整數輸出 <code>OK</code>，否則輸出 <code>BAD</code>。",
   "stdin": "101\n",
   "expect": "BAD"
  }
 }
